...
tml_file="Spotapps_Servicenow_Problem_Managment.worksheet.tml"
...
def rename_coulmn():
    ...
    # dups.to_csv("test.csv", index=False)
    final_recs = dups[(dups.match == 'N')]
    for index, row in final_recs.iterrows():
...
def worksheet_generation():
    file_json=out_path/"Spotapps_Servicenow_Problem_Managment.worksheet.json"
...
def os_cleanup():
    os.remove(out_path/"Spotapps_Servicenow_Problem_Managment.worksheet.json")
    os.remove(out_path/"Spotapps_Servicenow_Problem_Managment.worksheet.tml.bak")

def tml_success():
    f = open(out_path/"Spotapps_Servicenow_Problem_Managment.worksheet.json",'r')
